...
const getPending = async (connection, migrations) => {
  const data = await connection.query('select * from migrations')
...
const run = async (connection, type, model, version = '', migrations, sequelize) => {
  ...
  if (type === 'up') await connection.query(`insert into migrations (title) values ${toRun.map(migration => `('${migration.title}')`).join(', ')};`)
  if (type === 'down') await connection.query(`delete from migrations where title in (${toRun.map(migration => `'${migration.title}'`).join(', ')});`)
}
...
const runPending = async (connection, migrations, sequelize) => {
  ...
  if (pending.length > 0) {
    ...
    await connection.query(`insert into migrations (title) values ${pending.map(migration => `('${migration.title}')`).join(', ')};`)
  }
}
